...
import '../models/new_model.dart'; // For SessionBooking
...
class _MentorMatchingScreenState extends State<MentorMatchingScreen>
  ...
  Widget _buildMySessionsTab(bool isDark) {
    final userId = _currentUserId;
    if (userId == null) {
      return const Center(child: Text('Please sign in to view sessions'));
    }

    return StreamBuilder<List<SessionBooking>>(
      stream: _firestoreService.streamSessionsForStudent(userId),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
